CREATE DEFINER=`root`@`%` PROCEDURE `my_logs`(LimitPerPage int,pageNumber int,IN search varchar(255),IN userID varchar(255),isForward boolean,archiveList boolean)
BEGIN
...
where TChat_Log_Sender = userID and 
     ...
     if(search !='',SCON.name LIKE CONCAT('%',search ,'%') OR
      CR.HCHAT_ROOM_Name LIKE CONCAT('%',search ,'%') OR
       RUSERS.DN_PHONE LIKE CONCAT('%',search ,'%'),if(archiveList=true,TChat_Log_IS_Archive=true ,TChat_Log_IS_Archive=false) )
