...
module.exports = {
    ...
    // checkDeelKey:function(data,msgResponse,callback){
    //     nlpKeword(msgResponse, data.userId,msgResponse,function(err,message,type){
    ...
    checkDeelKey: function (data, msgResponse, callback) {
        // msgResponse.broadcast=data.broadcast
            ...
            .then((chatRoomRes) => {
                ...
                if (activeUsers.length == chatRoomRes.length) {
                    ...
                    MessageRecord.update({
                        ...
                        , TMESSAGES_Record_Updated_On: new Date().toISOString().slice(0, 19).replace('T', ' ')
                    }, {
                        where: {
                            TMESSAGES_Record_Chat_Room_ID: data.chatRoomId,
    ...
    updateRoomMessageStatus: function (data, status, callback) {
        Message.findAll({
            ...
            if (msgResp) {
                ...
                MessageRecord.update({
                    TMESSAGES_Record_Read_Status: status
                    , TMESSAGES_Record_Updated_On: new Date().toISOString().slice(0, 19).replace('T', ' ')
                }, {
                    where: data,
                    raw: true
            ...
            if (userRoomRes.length != 0) {
                ...
                for (var i = 0; i < roomMembers.length; i++) {
                    ...
                    if (picked.length == 0) {
                        ...
                        MessageRecord.update({
                            TMESSAGES_Record_Read_Status: 'Delivered'
                            , TMESSAGES_Record_Updated_On: new Date().toISOString().slice(0, 19).replace('T', ' ')
                        }, {
                            where: {
                                TMESSAGES_Record_Chat_Room_ID: data.chatRoomId,
    ...
    Notification.findAll({
        ...
        .then((notifyResp) => {
            if (notifyResp.length != 0) {
                ...
                if (data.isGroup) {
                    notifyData = {
                        "Receiver": receiverUserDetails.DN_ID,
    ...
    Message.find({
        ...
        Message.create(createmessage).then((msgRep) => {
            ...
            if ((createdMessage.TMESSAGES_File_Type == "text" || createdMessage.TMESSAGES_File_Type == "Text") && !data.isBux) {
                // createdMessage.broadcast=true
                nlpKeword(createdMessage, data.userId, createdMessage, function (err, message, type) { })
            }
...
function sendPushNotification(data, onlineMember, receiver, notification_type, callback) {
    if (data.istype == "image") {
        ...
        messageContent = "📹 Video";
    } else if (data.isBux === true) {
        messageContent = "Dazz Recevied ";
    } else {
    ...
    if (data.isGroup) {
        ...
        notifyData = {
            ...
            "description": receiver.name + ": " + messageContent,
    ...
    if (receiver.isMute == 0 || receiver.isMute == false) {
        rp({
            ...
            .then((response) => {
                if (response.status == 200 && receiver.isMute == 0 || receiver.isMute == false) {
                    MessageRecord.update({
                        TMESSAGES_Record_Read_Status: "Delivered"
                        , TMESSAGES_Record_Updated_On: new Date().toISOString().slice(0, 19).replace('T', ' ')
                    }, {
...
function createRoomMessage(onlineMember, createMsgJson, data, callback) {
    ...
    if (data.isReply) {
        createMsgJson.TMESSAGES_IS_Reply = true,
            ...
            createMsgJson.TMESSAGES_Reply_ID = data.replayMsgId,
    ...
    Message.find({
        ...
        Message.create(createMsgJson).then((msgRep) => {
            ...
            Message.find({
                where: {
                    TMESSAGES_Message_ID: createdUserMessage.TMESSAGES_Message_ID,
                },
            }).then(msgdata => {
                createdUserMessage = JSON.parse(JSON.stringify(msgdata))
                ...
                message_Record.TMESSAGES_Record_Message_ID = createdUserMessage.TMESSAGES_Message_ID,
                    message_Record.TMESSAGES_Record_Read_Status = "Read",

                    MessageRecord.create(message_Record).then(res => {
                        User.find({
                            where: {
                                DN_ID: createdUserMessage.TMESSAGES_UID
                            },
                            include: [
                                {
                            ...
                            if (data.isBroad != true) {
                                ...
                                //     nlpKeword(msgRep, data.userId,createdUserMessage)
                                // }
                            }
                                    ...
                                    if (res.length == 0) {
                                        ...
                                        return Chatroom.findAll({
                                            ...
                                            .then((chatRoomResData) => {
                                                ...
                                                if (chatRoomResData[1]) {
                                                    chatLogInput.TChat_Log_Sender = chatRoomResData[1]['chatRoomData.DUSER_ROOM_UID']
                                                    chatLogInput.TChat_Log_Receiver = chatRoomResData[0]['chatRoomData.DUSER_ROOM_UID']

                                                    Chatlog.create(chatLogInput).then((chatLogResponse) => { });

                                                    chatLogInput.TChat_Log_Sender = chatRoomResData[0]['chatRoomData.DUSER_ROOM_UID']
                                                    chatLogInput.TChat_Log_Receiver = chatRoomResData[1]['chatRoomData.DUSER_ROOM_UID']
                                                    Chatlog.create(chatLogInput).then(res => { });
                                                }
...
function createMessageRecordLogs(inputData, createdMessage, isBroadCast, callback) {
    ...
    Chatroom.findAll({
        ...
        console.log(JSON.parse(JSON.stringify(chatRoomRes)), '##################################################################################################')
        return _.forEach(chatRoomRes,function (chatRoomRes, index) {
            console.log(index, "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
            ...
            if (userId != inputData.userId && !chatRoomRes['chatRoomData.DUSER_ROOM_IS_User_left']) {
                ...
                MessageRecord.findAll({
                    ...
                    userMessage.TMESSAGES_Record_UID = userId
                    console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$", userMessage.TMESSAGES_Record_UID);
                    return MessageRecord.create(userMessage).then(res => {
                        console.log(JSON.parse(JSON.stringify(res)), ')(()sfgfd*&*&)$$$$$$$$$$$');
                    })
                    // return true;
                })
            }
...
//NLP keywork
function nlpKeword(message, userId, createdUserMessage, callback) {
    Userroom.findAll({
        where: {
            DUSER_ROOM_Chat_Room_ID: createdUserMessage.TMESSAGES_Chat_Room_ID,
        ...
        console.log("!!!!!!!!!!!!!!!!!!!!!!!!! notification for deelkey to users", roomUsers)
        // if(message.dontSendNotification===true){
        //     roomUsers=[]
        // }
        let data = {
            "loginuser": userId,
            "otherUsers": roomUsers,
            "message": createdUserMessage.TMESSAGES_Content
        }
        console.log(data)

        rp({
            url: config.DeelchatAPI + 'deelChat/nlpkeywords',
            'Content-type': 'application/json',
            ...
            .then((response) => {
                ...
                if (response.status != null) {
                    if (response.status.status == 200) {
                        let nlpKey = response.entity.keywords[0]
                        let userbasedKeywords = response.entity.userbasedKeywords;
                        Message.update({ TMESSAGES_IS_Deel_Keyword: nlpKey }, {
                            where: {
                                TMESSAGES_Message_ID: createdUserMessage.TMESSAGES_Message_ID
                            }
                        }).then(response => {
                            createdUserMessage.TMESSAGES_IS_Deel_Keyword = nlpKey
                            createdUserMessage.userbasedKeywords = userbasedKeywords
                            callback(null, createdUserMessage)
                        })
                    } else {
                        createdUserMessage.TMESSAGES_IS_Deel_Keyword = null
                    }
                }
            });
    })
