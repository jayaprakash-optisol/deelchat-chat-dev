...
let connectionQuery = mysql.createConnection(sqlConnection);
...
router.get('/my_logs',(req, res, next) => {
  console.log(req.query.archiveList, "####")
  ...
  let archiveList = req.query.archiveList
  if(archiveList==1){
    archiveList=true
  }else{
    archiveList=false
 ...
 let setQuery=`CALL my_logs(${limit},${offset},${searchQuery},${UseriD},${false},${archiveList})`
//  connectionQuery.query(setQuery, function (err, response) {
//   })
...
//   updateNotification(req.query.userId)
...
config.raw([setQuery]).then((response) => {
  ...
  if(response){
   ...
   let msgList = finalObject[1];
   console.log(msgList, '####');
   if(msgList.length>0){
    msgList = _.uniqBy(msgList, 'TChat_Log_Chat_Room_ID');
   }
   res.setHeader("statusCode", 200);
      res.status(200).json({
        status: "Success",
