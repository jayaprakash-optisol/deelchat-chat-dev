...
angular
  ...
  .directive('schrollBottom', function () {
    // custom directive for scrolling bottom on new message load
    return {
      scope: {
        schrollBottom: '=',
      },
  ...
  .directive('focusMe', function ($timeout) {
    // custom directive for focusing on message sending input box
    return {
    ...
    function (
      ...
      $scope.toggleCustom = function () {
        ...
        if (!$scope.custom) {
          if (
            !angular
              .element(document.querySelector('#slidememberlist'))
              .hasClass('slideout_inner_trans')
          ) {
            angular
              .element(document.querySelector('#slidememberlist'))
              .addClass('slideout_inner_trans');
          }
        } else {
          if (
            angular
            ...
            angular
              .element(document.querySelector('#slidememberlist'))
              .removeClass('slideout_inner_trans');
          }
        }
      };
      ...
      $scope.userTypeing = function () {
        ...
        $socket.emit(
          'user-typing',
          { username: $rootScope.username, roomname: $rootScope.roomname },
          function (data) {}
        );
        setTimeout(function () {
      ...
      $socket.on('user-stop-typing-response', function (data) {
        console.log(data.username + ' is stoppppppppp typing');
      });
      ...
      $scope.openClickImage = function (msg) {
        if (!msg.ownMsg) {
          $http
            .post($rootScope.baseUrl + '/v1/getfile', msg)
          ...
          if ($scope.messeges[i].hasFile) {
            if ($scope.messeges[i].istype === 'image') {
              if ($scope.messeges[i].dwid === search_id) {
                $http
                  .post($rootScope.baseUrl + '/v1/getfile', $scope.messeges[i])
                  .success(function (response) {
                    if (!response.isExpired) {
        ...
        if (files && files.length) {
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            $socket.emit('send-message', image, function (data) {
              // sending new image message via socket
              console.log('@@@@@ @@@@@@@@@@');
              // console.log(data)
            });
            var fd = new FormData();
      ...
      $scope.openClickMusic = function (msg) {
        $http
          ...
          .success(function (response) {
            if (!response.isExpired) {
              window.open(
                $rootScope.baseUrl + '/' + response.serverfilename,
                '_blank'
              );
            } else {
              var html = '<p id="alert">' + response.expmsg + '</p>';
          ...
          if ($scope.addedusers != null && $scope.addedusers.length > 0) {
            ...
            $socket.emit('create-group', grpInp, function (data) {
              ...
              if (data.success == true) {
                ...
                $socket.emit(
                  'get-group',
                  { userId: 1, username: $rootScope.username },
                  function (data) {
